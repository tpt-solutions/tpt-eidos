//! tpt-eidos medical dosing-bounds domain library.
//!
//! Provides pre-proved medical dosing primitives: dose clamping, rate
//! application, and dose splitting. Uses the same `Lemma`/`TrustedLemmas`
//! pattern as `tpt-eidos-controls-math` for extensible non-linear obligations.
//!
...
/// The reusable medical dosing primitives, as eidos source. Feed this to
/// `parse` and `check_module` to confirm the domain library verifies.
pub const PRIMITIVES_EIDOS: &str = include_str!("primitives.eidos");

/// Domain-specific lemmas for the medical dosing verification environment.
/// Currently empty; the kernel's `DEFAULT_LEMMAS` cover all linear arithmetic
/// obligations in the primitives. This set is where additional,
/// medical-domain-specific trusted facts are registered as the library grows.
pub static MEDICAL_LEMMAS: &[Lemma] = &[];
...
fn combined(extra: &[Lemma]) -> Vec<Lemma> {
    ...
    v.extend(MEDICAL_LEMMAS.iter().copied());
    v.extend(extra.iter().copied());
    v
}

/// Verify a medical dosing module with the standard domain-library lemma set.
pub fn check_module(module: &Module) -> Report {
    check_with(module, &combined(&[]))
}

/// Verify a medical dosing module, additionally trusting the given extra
/// lemmas.
pub fn check_module_with(module: &Module, extra: &[Lemma]) -> Report {
    check_with(module, &combined(extra))
}

/// Parse and verify a medical dosing eidos source string.
